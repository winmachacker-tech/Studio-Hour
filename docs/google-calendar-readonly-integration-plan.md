...
**Why this scope**:
- Read-only access to calendar list, calendar metadata, and events
- Required for `calendarList.list` — without it, Studio Hour cannot discover shared/family calendars and returns 403
- The previous scope `calendar.events.readonly` could read events on a known calendar ID but could not list the user's calendars, making it impossible to find shared calendars
- Still fully read-only — the app cannot create, modify, or delete events or calendars
- If write access is needed later (e.g., blocking focus time), it can be requested as a scope upgrade

**Scope history**:
- V1 initially used `calendar.events.readonly` — insufficient for shared/family calendars (calendarList 403)
- Upgraded to `calendar.readonly` to support multi-calendar discovery

**Not recommended**:
- `calendar` (full read-write) — unnecessary
- `calendar.events.readonly` — insufficient for calendarList.list

**Reconnect required**: Existing users who connected under the old scope must reconnect Google Calendar to grant `calendar.readonly`. The app's reconnect flow handles this — tapping "Reconnect" in Settings triggers a fresh OAuth consent with the new scope.
...
- [x] Add scope: `https://www.googleapis.com/auth/calendar.readonly` (upgraded from `calendar.events.readonly` to support calendarList)
- [x] Create Android OAuth client: Studio Hour Android Debug (`com.studiohour.app`)
- [ ] Note: production distribution requires Google app verification for `calendar.readonly` scope
...
1. **Google OAuth consent screen verification** — `calendar.readonly` is a restricted scope. Google requires verification for apps with >100 users. For a single-user app this may not matter immediately, but could block broader distribution later.
